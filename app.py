...
@app.route('/api/hero/<hero_name>', methods=['GET'])
def api_hero(hero_name):
    """API: получить данные героя"""
    logger.info(f"API запрос героя: {hero_name}")
    hero_data = get_hero_data(hero_name)
    if not hero_data:
        return jsonify({'error': 'Hero not found'}), 404
    return jsonify(hero_data)

@app.route('/api/heroes/stats', methods=['GET'])
def get_heroes_stats_api():
    """API: получить статистику героев"""
...
def get_heroes_list():
    """Получить список героев из OpenDota API"""
    ...
    try:
        response = requests.get(f'{OPENDOTA_API_BASE}/heroes', timeout=10)
        if response.status_code == 200:
            data = response.json()
            set_cache(cache_key, data)
            return data
    except Exception as e:
        logger.error(f"Ошибка получения списка героев: {e}")
    
    return []

def get_heroes_stats():
    """Получить статистику героев из OpenDota API"""
    cache_key = 'heroes_stats'
    cached = get_cache(cache_key)
    if cached:
        return cached
    
    try:
        response = requests.get(f'{OPENDOTA_API_BASE}/heroStats', timeout=10)
        if response.status_code == 200:
            data = response.json()
            set_cache(cache_key, data)
            return data
    except Exception as e:
        logger.error(f"Ошибка получения статистики героев: {e}")
    
    return []

def get_hero_data(hero_name):
    """Получить данные конкретного героя"""
    stats = get_heroes_stats()
    for stat in stats:
            ...
            if hero_id:
                try:
                    abilities_response = requests.get(
                        f'{OPENDOTA_API_BASE}/heroes/{hero_id}/abilities', 
                        timeout=5
                    )
                    if abilities_response.status_code == 200:
                        abilities = abilities_response.json()
                except Exception as e:
                    logger.warning(f"Не удалось загрузить способности: {e}")
...
def format_news_content(text):
    """Форматировать содержимое новости в HTML"""
    lines = text.split('\n')
    html_parts = []
    in_list = False
    list_items = []
...
def initialize_news():
    """Инициализировать новости при запуске"""
    print("=" * 60)
...
# Запускаем инициализацию новостей
initialize_news()
