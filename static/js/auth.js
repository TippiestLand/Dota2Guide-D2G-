(function() {
    ...
    function updateUI(user) {
        if (user) {
            ...
            if (adminBtn && user.role === 'admin') adminBtn.style.display = 'block';
    ...
    function checkSavedUser() {
        try {
            ...
            if (saved) {
                ...
                fetch('/api/verify', {
                    headers: { 'X-Admin-Auth': user.token }
                })
                .then(res => res.json())
                .then(data => {
                    if (data.valid) updateUI({ ...user, ...data });
                    else updateUI(null);
                })
                .catch(() => updateUI(null));
            }
    ...
    registerForm.addEventListener('submit', async function(e) {
        ...
        if (password.length < 6) { showNotification('❌ Пароль должен быть минимум 6 символов', 'error'); return; }
        try {
            ...
            if (res.ok) {
                closeAuth();
                updateUI({ username: data.user.username, email: data.user.email, role: data.user.role, token: data.token });
                setTimeout(() => { document.querySelector('[data-page="home"]')?.click(); }, 500);
            } else {
    ...
    openAuthBtn.addEventListener('click', function() {
        if (currentUser) {
            if (confirm('Выйти из аккаунта?')) window.logout();
        } else { openAuth(); }
    });
